...
interface OutcomeStatementDeleteDialogProps {
  open: boolean;
...
export function OutcomeStatementDeleteDialog({
  open,
  onOpenChange,
  statementText,
  isDeleting,
  onConfirm,
}: OutcomeStatementDeleteDialogProps) {
      ...
      <DialogContent className="max-w-2xl rounded-[28px] border border-border/80 bg-card/98 p-0 shadow-[var(--shadow-elevated)]">
        <DialogHeader className="border-b border-border/70 px-8 py-6 text-left">
          ...
          <DialogTitle className="text-2xl font-semibold tracking-tight">
            {locale.outcomeStatementDelete.title}
          </DialogTitle>
          <DialogDescription className="mt-2 text-sm leading-6 text-muted-foreground">
            {locale.outcomeStatementDelete.description.replace(
              "{{statement}}",
              statementText ?? "",
            )}
          </DialogDescription>
        ...
        <DialogFooter className="border-t border-border/70 px-8 py-5">
          ...
          >
            {isDeleting
              ? locale.outcomeStatementDelete.deleting
              : locale.outcomeStatementDelete.confirmAction}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
