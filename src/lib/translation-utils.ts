...
export function translateRole(t: TFunction, role: 'owner' | 'member') {
...
export function translateStatus(
  ...
  const translationKey = status ? statusKeyMap[status as keyof typeof statusKeyMap] : undefined;
...
export function translatePrivacyCategory(
  t: TFunction,
  category: 'Direct Identifier' | 'Quasi Identifier' | 'High Risk' | 'Operational' | 'Outcome',
) {
  const keyMap = {
