...
import { useCreateInvitationMutation, useOrganizationInvitationsQuery } from "@/hooks/use-grantready";
  ...
  return (
    ...
    >
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-semibold tracking-tight">{t("auth.inviteCardTitle")}</h2>
          <p className="mt-2 text-sm leading-6 text-muted-foreground">{t("auth.inviteCardDescription")}</p>
        </div>
        ...
        <form className="space-y-4" onSubmit={invite}>
          ...
          <Button type="submit" className="w-full" disabled={createInvitationMutation.isPending}>
            {createInvitationMutation.isPending ? t("auth.inviting") : t("auth.sendInvitation")}
          </Button>
        </form>

        {(invitationsQuery.data ?? []).filter((invitation) => invitation.status === "pending").length > 0 ? (
          <div className="space-y-3">
            {(invitationsQuery.data ?? [])
              .filter((invitation) => invitation.status === "pending")
              .map((invitation) => (
                <div key={invitation.id} className="rounded-2xl border border-border bg-secondary/20 px-4 py-3 text-sm">
                  <div className="font-medium text-foreground">{invitation.email}</div>
                  <div className="text-xs text-muted-foreground">{t("auth.pendingInvitation")}</div>
                </div>
              ))}
          </div>
        ) : null}
