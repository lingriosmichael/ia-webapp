    ...
    <>
      ...
      <div className="mx-auto w-full max-w-6xl px-8 py-10">
        <PageHeader
          ...
          description={t("projectAnalytics.description", {
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-2">
          ...
          >
            <ResponsiveContainer>
              <LineChart data={confidenceImprovement}>
                <CartesianGrid
                  stroke="oklch(0.93 0.005 280)"
                  vertical={false}
                />
          ...
          <ChartCard title={t("projectAnalytics.charts.completionByAgeGroup")}>
            <ResponsiveContainer>
              <BarChart data={completionByAgeGroup} stackOffset="expand">
                <CartesianGrid
                  stroke="oklch(0.93 0.005 280)"
                  vertical={false}
                />
          ...
          <ChartCard title={t("projectAnalytics.charts.mentorMatching")}>
            <ResponsiveContainer>
              <PieChart>
                <Tooltip contentStyle={tooltipStyle} />
                <Pie
          ...
          <Card className="p-5">
            <h3 className="text-[14px] font-semibold tracking-tight">
              {t("projectAnalytics.charts.backendStatus")}
            </h3>
            <ul className="mt-4 space-y-3 text-[13px]">
              <li className="flex items-center justify-between border-b border-border/60 pb-2">
                <span className="text-muted-foreground">
                  {t("projectAnalytics.backendStatus.projectLoaded")}
                </span>
                <span className="font-medium">
                  {t("projectAnalytics.backendStatus.yes")}
                </span>
              </li>
              <li className="flex items-center justify-between border-b border-border/60 pb-2">
                <span className="text-muted-foreground">
                  {t("projectAnalytics.backendStatus.activitiesLoaded")}
                </span>
                <span className="font-medium">{activities.length}</span>
              </li>
              <li className="flex items-center justify-between">
                <span className="text-muted-foreground">
                  {t("projectAnalytics.backendStatus.aiAnalytics")}
                </span>
                <span className="font-medium">
                  {t("projectAnalytics.backendStatus.notImplemented")}
                </span>
              </li>
            </ul>
          </Card>
