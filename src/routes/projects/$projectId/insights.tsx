    ...
    <>
      ...
      <div className="mx-auto w-full max-w-5xl px-8 py-10">
        <PageHeader
          ...
          description={t("projectInsights.description", {
            count: activities.length,
          })}
        />
        <Card className="mt-8 border-primary/15 bg-gradient-to-br from-primary-soft to-card p-6">
        ...
        <Card className="mt-6 border-success/30 bg-[oklch(0.98_0.02_155)] p-6">
          <div className="flex items-start gap-3">
            <div className="grid h-9 w-9 place-items-center rounded-lg bg-success/15 text-[oklch(0.4_0.12_155)]">
              <ShieldCheck className="h-4 w-4" />
            </div>
            <div>
              <h3 className="text-[15px] font-semibold tracking-tight">
                {t("projectInsights.privacyTitle")}
              </h3>
              <p className="mt-0.5 text-[12px] text-muted-foreground">
                {t("projectInsights.privacyDescription")}
              </p>
            </div>
          </div>
          <ul className="mt-5 grid gap-2.5 md:grid-cols-3">
            {projectInsights.privacy.map((item, index) => (
              <li
                key={index}
